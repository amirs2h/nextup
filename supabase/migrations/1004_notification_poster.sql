-- ============================================================
-- MIGRATION 1004: Add poster_path to notification data
-- ============================================================
-- Stores poster_path in data JSONB so the notification card
-- can show a small poster of the movie/show alongside the avatar
-- ============================================================

-- 1. notify_followers_on_comment: add poster_path
...
-- 2. notify_on_reply: add poster_path
...
-- 3. notify_on_comment_like: add poster_path
...
BEGIN
  SELECT user_id, content, title, poster_path, tmdb_id, media_type, season_number, episode_number
  INTO comment_owner_id, comment_content, comment_title, comment_poster, comment_tmdb_id, comment_media_type, comment_season, comment_episode
  ...
  IF comment_owner_id IS NOT NULL THEN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'title', COALESCE(comment_title, 'a movie/show'),
        'poster_path', comment_poster
      )
    );
  END IF;
...
-- 4. notify_on_follow: add poster_path (not applicable, but ensure avatar_url exists)
...
-- 5. Add poster_path column to comments table (if not exists)
ALTER TABLE public.comments ADD COLUMN IF NOT EXISTS poster_path TEXT;
